...
if len(sys.argv)==7:
    ...
    wgt.color_print('''##### ERROR: use no parmaters or 6 parameters as foolowing:
    par1: ny1
    par2: nz1
    par3: ny2
    par4: nz2
    ...
    i = 0
    for inputInArgv in sys.argv:
        print('par%d: %s' % (i, inputInArgv) )
        i+=1
...
#ny1, nz1 = 61, 61
#ny2, nz2 = 61, 61
#nl, nw = 51, 2001  # try to have same dw and dl
...
l, w = np.mgrid[-.5e-3:.5e-3:nl*1j, -10e-3:10e-3:nw*1j]

R, rho = wgt.curvatureRadiusToroid(r, rp, alpha=alpha)

print("WG: R = %.3g m, rho = %.3g m" % (R, rho))
h_lw = toroidSurface(w, l, R=R, rho=rho)
...
if rank == 0:           # because the arrays have different lenghts I need
    ...
    filename = 'fk_' + wgt.datetimeNowStr() + '_mpi'
    ...
    title = str(r'$N_1^2 \times N_2^2 \times n_l \times n_w \,=\, %d^2 \times %d^2 \times %d \times %d, r = %.1fm, rp = %.1f$' % (ny1, ny2, nl, nw, r, rp))
